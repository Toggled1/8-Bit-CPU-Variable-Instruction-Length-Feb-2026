...
For short instructions:     FETCH0 -> FETCH0_BUF -> DECODE0 -> DECODE0_BUF -> LOADOPS -> EXECUTE

For long instructions:      FETCH0 -> FETCH0_BUF -> DECODE0 -> DECODE0_BUF -> FETCH1 -> FETCH1_BUF 
...
Instruction Info
------------------------------------
imm is applicable to 2-byte instructions

Byte0:  7 6 5 4  3  2 1 0
        X X X X imm A A A          (A = R[A] sel)


(imm=0) Byte1:  7 6 5 4 3 2 1 0
                B B B              (BBB = R[B] sel) (src = R[B])

(imm=1) Byte1:  7 6 5 4 3 2 1 0
                Y Y Y Y Y Y Y Y (YYY... = addr sel) (src = MEM[YYY...])



XXXX	OP	    Length	    SCHEME
